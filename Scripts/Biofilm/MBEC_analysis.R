...
filename1 = "MBEC_epidermidis_experimento_1_compuesto_C1_more_shaking_30_seg_300rpm.csv"
filename2 = "MBEC_epidermidis_experimento_1_compuesto_C1_more_shaking_30_seg_300rpm_replica2.csv"
filename3 = "MBEC_epidermidis_experimento_1_compuesto_C1_more_shaking_30_seg_300rpm_replica3.csv"
...
raw_data1 =  raw_data1[c(1:4,8),]
raw_data2 =  raw_data2[c(1:4,8),]
raw_data3 =  raw_data3[c(1:4,8),]
...
#================================================================================
bacteria_condition =  c("Replicate_1","Replicate_2","Replicate_3","Negative_control","Positive_control")
...
conditions <- paste('Condition', sprintf("%02d", 1:5), sep = "_")
replacements <- bacteria_condition[1:5]
...
calculate_mean_negative_control <- function(dataframe) {
  ...
  mean_negative_control <- tapply(Tabla_Ensayo_control_negativo$Absorbance, factor(Tabla_Ensayo_control_negativo$Condition), mean)
...
process_dataframe <- function(dataframe) {
  ...
  Tabla_Ensayo <- subset(dataframe, Condition %in% c("Replicate_1", "Replicate_2", "Replicate_3", "Positive_control"), select = c(concentration, Absorbance, Condition))
  ...
  # Filter rows
  Tabla_Ensayo <- Tabla_Ensayo[c(1:36, 44:46), ]
...
mean_MBEC = list(mean_absorbance_1, mean_absorbance_2, mean_absorbance_3)
...
#=====================================================================================
#Means
  mean_MBEC <- tapply(melted_df$Mean, factor(melted_df$Concentration), na.rm = TRUE, mean)

#SDs 
  sd_MBEC <- tapply(melted_df$Mean, factor(melted_df$Concentration), na.rm = TRUE, sd)
...
title = "MBEC S. epidermidis C1 3 Replicates"
significances = c("a", "ab", "ab", "ab", "ab", "ab", "abc", "abc", "abc", "bc", "ab", "abc", "c")
...
p <- ggplot(data=df_plot, aes(x=Conc, y=mean)) + 
          ...
          labs(title=element_blank(), x=expression(paste("Compound concentration (", mu, "M)")), y =  "Growth after 24 hs (OD_600)")+
...
title = "MBEC S. epidermidis C1 3 Replicates"
#significances = c("a", "ab", "ab", "ab", "ab", "ab", "cd", "abc", "abc", "d", "ab", "bcd", "bcd")
#significances = c("a", "bc", "bc", "bc", "bc", "bc", "ab", "abc", "abc", "bc", "c", "d", "d")
#significances = c("a", "ab", "abc", "a", "abc", "abc", "abc", "abcd", "d", "bcd", "abcd", "cd", "e")
significances = c("a", "ab", "ab", "ab", "ab", "ab", "abc", "abc", "abc", "bc", "ab", "abc", "c")
	...
	p <- ggplot(data=df_plot, aes(x=Conc, y=Mean)) + 
	            theme_bw() +
	  			...
	  			geom_errorbar(aes(ymin=Mean_i-SD_i, ymax=Mean_i+SD_i), width=0.2,
	  			    ...
	  			    theme(axis.text.x = element_text(color="black", size=12, angle=10)) +
	      			theme(legend.position="top",legend.title = element_text(size = textSize), legend.text  = element_text(size = textSize)) + 
	                     labs(title=element_blank(), x=expression(paste("Compound concentration (", mu, "M)")), y = "Growth after 24 hs (OD)")+
...
#Tabla_Ensayo_control_positivo = subset(processed_data_list_1, Condition =="Positive_control", select=c(Condition,concentration, Absorbance))

#Tabla_Ensayo_control_positivo = Tabla_Ensayo_control_positivo[c(7:12),]

#mean_positive_control=tapply(Tabla_Ensayo_control_positivo$Absorbance,factor(Tabla_Ensayo_control_positivo$Condition),mean)
...
#apply the log10 funtion mean

fun = function(x){ log10(x)}

log10_mean = lapply(mean_CFU, fun)

#apply the log10 funtion sd #SD = sqrt(a^2 + a^2)

fun = function(x){ log10(x)}

log10_sd = lapply(sd_CFU, fun)




mean_of_mean_LOG10 = (log10_mean[[1]] + log10_mean[[2]] + log10_mean[[3]])/3

sd_of_sd_LOG10 = sqrt(((log10_sd[[1]])^2 + (log10_sd[[2]])^2 + (log10_sd[[3]])^2)/3)
...
#obliga a ordenar a que el eje x se ordene de manera equidistante
df_plot_log$Conc <- factor(df_plot_log$Conc, levels = df_plot_log$Conc)

title = "CFU S. epidermidis C1 Three Replicates "

p <- ggplot(data=df_plot_log, aes(x=Conc, y=Log10_mean)) + 
	            theme_bw() +
	  			geom_bar(stat="identity",  width=0.9, fill = "red")+ 
	  			ylim(0, 6)+
	  			geom_errorbar(aes(ymin=Log10_mean-SD, ymax=Log10_mean+SD), width=0.2)+
...
raw_data$CFU.ml[raw_data$CFU.ml == 0] <- NA

raw_data$CFU.ml = log10(raw_data$CFU.ml)

#aplico log 10 por consejo de Margarita


mean_CFU <- tapply(raw_data$CFU.ml,factor(raw_data$Concentration_number),na.rm = TRUE,mean)
sd_CFU <- tapply(raw_data$CFU.ml,factor(raw_data$Concentration_number),na.rm = TRUE,sd)
...
title = "CFU S. epidermidis C1 Replica 3"
